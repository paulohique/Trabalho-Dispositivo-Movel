...
def processar_gabarito(filepath, gabarito_correto):
    ...
    todos_os_centros = np.array(
        [[cv2.moments(c)['m10'] / cv2.moments(c)['m00'], cv2.moments(c)['m01'] / cv2.moments(c)['m00']]
         for c in candidatos_circulares if cv2.moments(c)['m00'] > 0],
        dtype="float32"
    )
    ...
    pontos_np = np.array([
        todos_os_centros[indice_tl],
        todos_os_centros[indice_tr],
        todos_os_centros[indice_br],
        todos_os_centros[indice_bl]
    ], dtype="float32")
    ...
    for i in range(10):
        if i < len(linhas) and len(linhas[i]) >= 5:
            ...
            if pixels_nao_zeros:
                ...
                indices_marcados = [idx for idx, val in enumerate(pixels_nao_zeros) if val > limiar_marcacao]

                if len(indices_marcados) == 1:
                    indice_marcado = indices_marcados[0]
                    ...
                    for indice in indices_marcados:
                        info = linha_bolhas[indice]
                        cv2.rectangle(debug_analise,
                                      (info['x'], info['y']),
